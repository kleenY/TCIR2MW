...
from models.my_fca_attention import MultiSpectralAttentionLayer
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3,input_dim=3,old = 1):
        ...
        n_patches = (img_size[0] // patch_size_real[0] +1) * (img_size[1] // patch_size_real[1] +1)
        in_channels = 1024
...
class ART_block(nn.Module):
    ...
    def forward(self, x):
        if self.transformer:
            ...
            transformer_out = F.interpolate(transformer_out, size=(transformer_out.shape[2] - 1, transformer_out.shape[3] - 1), mode='bilinear',
                              align_corners=False)
